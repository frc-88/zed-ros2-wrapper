...
YoloDetector::YoloDetector(const std::string& model_path, const torch::DeviceType& device_type, bool report_loop_times) : device_(device_type) {
...
YoloDetector::Run(const cv::Mat& img, float conf_threshold, float iou_threshold) {
...
std::vector<std::vector<Detection>> YoloDetector::PostProcessing(const torch::Tensor& detections,
...
void YoloDetector::ScaleCoordinates(std::vector<Detection>& data,float pad_w, float pad_h,
...
void YoloDetector::Tensor2Detection(const at::TensorAccessor<float, 2>& offset_boxes,
...
std::vector<std::string> YoloDetector::LoadNames(const std::string& path) {
    // load class names
